...
from binary_units import HardBinaryConv, BinaryActivation
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, bias=False, downsample=None, dilation=1):
        ...
        self.binary_conv = HardBinaryConv(in_channels, out_channels, stride=stride,
                                          ...
                                          padding=padding,
                                          dilation=dilation
                                          ) # bias is False by deffault i think
...
def bidet_vgg(cfg, i=3, bias=False):
    ...
    conv6 = HardBinaryConv(512, 1024, kernel_size=3, padding=6, dilation=6)
...
class BiDetSSD(nn.Module):
    ...
    def __init__(self, phase, size, base, extras, head, num_classes,
        ...
        self.act = nn.ModuleList([BinaryActivation() for i in range (len(self.extras))])
    ...
    def forward(self, x):
        ...
        for i in range(len(self.extras)):
            ...
            x = self.extra_bns[i](self.extras[i](  self.act[i](x)    ))
            if i % 2 == 1:
                sources.append(x)
...
def add_extras(cfg, i):
    ...
    for k, v in enumerate(cfg):
        if in_channels != 'S':
            if v == 'S':
                layers += [HardBinaryConv(in_channels, cfg[k + 1],
                                          kernel_size=(1, 3)[flag], stride=2, padding=1,
                                          bias=False)]
            else:
                layers += [HardBinaryConv(in_channels, v, kernel_size=(1, 3)[flag],
...
def multibox(vgg, extra_layers, cfg, num_classes):
    ...
    for k, v in enumerate(vgg_source):
        #print('multibox', k, v, vgg[v])
        try:
            channel = vgg[v].out_channels
        except:
            channel = vgg[v].conv.out_channels
        loc_layers += [nn.Conv2d(channel, cfg[k] * 4, kernel_size=3, padding=1)]
...
def build_binary_ssd(phase, size=300, num_classes=21,
...
if __name__=='__main__':
      import torch.nn as nn
      ssd = build_binary_ssd("train", 300, 81)

      x = torch.randn(32, 3, 300, 300)
      x = x.cuda()
      ssd = nn.DataParallel(ssd).cuda()
      print(ssd(x))
